...
export default function AppSidebar({ open, onOpenChange }: AppSidebarProps) {
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange} direction="left">
      <DrawerContent className="bg-white border-r-2 border-black p-0 gap-0 w-72 sm:max-w-none h-full flex flex-col justify-between">
        <DrawerHeader className="sr-only">
          <DrawerTitle>Menu</DrawerTitle>
          <DrawerDescription>Navigasi utama KeRaS</DrawerDescription>
        </DrawerHeader>
        ...
        <div className="flex flex-col flex-1 min-h-0">
          ...
          <ul className="flex w-full min-w-0 flex-col gap-1 pt-4 overflow-y-auto flex-1">
            {items.map((item) => {
        ...
        {/* Menu paling bawah (fixed in bottom) mengarah ke landing page ("/") */}
        <div className="p-2 border-t-2 border-black mt-auto">
          <Link
            href={isHomeActive ? "#" : "/"}
            onClick={() => onOpenChange(false)}
            className={cn(
